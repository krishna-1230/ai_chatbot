...
# model.add(Dense(64, activation='relu'))  # You can adjust the number of neurons as needed
...
# Train the model
model.fit(training_data, output_data, epochs=1000, batch_size=50, verbose=1)
...
def predict_intent(text):
    ...
    for sentence in sentences:
        ...
        if np.max(result) > threshold:
            ...
            intent = "unknown"
        outputt.append(intent)
    output = "".join(outputt)
    return output
# Example usage:
# predicted_intents = predict_intent("What's your favorite video game? Can you recommend a good game?")
...
@app.route('/home')
def home1():
    """ this is the first page after login or signup . still need to edit the content "UNDER PROCESS" """
    return render_template("main1.html")


@app.route('/about')
def about():
    """ this page displays about our team. still need to edit the content "UNDER PROCESS" """
    return render_template("about.html")


@app.route('/contact')
def contact():
    """ this is the contact page . this will go to the function below after clicking the "send" button """
    return render_template("contact.html")


@app.route('/thankyou', methods=['POST'])
def contact_post():
    """ this gets the name, password, phone.no and email and sends it to a gmail ID through the "send_mail_function"
    function and renders the thankyou_contact.html page , which has a button to return to "main1.html" which is the
    main page after login or signup """
    if request.method == 'POST':
